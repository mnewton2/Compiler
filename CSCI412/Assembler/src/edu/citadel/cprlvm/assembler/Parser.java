...
public class Parser
  {
        ...
        switch (opCode.getSymbol())
          {
            ...
            case LOADCB:  return new InstructionLOADCB(labels, opCode, arg);
            ...
            case LDADDR:  return new InstructionLDADDR(labels, opCode, arg);
            case LDGADDR: return new InstructionLDGADDR(labels, opCode, arg);
            case LDMEM:   return new InstructionLDMEM(labels, opCode, arg);
            ...
            default:
                ...
                InternalAssertion.check(false,   // always fails
                    "Parser.makeInstruction():  opcode " + opCode.getSymbol()
                    + " at position " + opCode.getPosition() + " not handled ");
                return null;
    ...
    private void checkOpCode() throws ParserException
      {
        if (!scanner.getSymbol().isOpCode())
          {
            String errorMessage = "Expecting an opcode but found \""
    ...
    private void checkArgs(Token opCode, Token arg)
        throws ParserException
      {
        ...
        if (numArgs == 0)
          {
            if (arg != null)
              {
                String   errorMessage  = "No arguments allowed for this opcode.";
                throw new ParserException(errorPosition, errorMessage);
              }
          }
        else if (numArgs == 1)
          {
            if (arg == null)
              {
                String   errorMessage  = "One argument is required for this opcode.";
                throw new ParserException(errorPosition, errorMessage);
              }
          }
        else
          {
            String   errorMessage  = "Invalid number of arguments for opcode " + opCode  +".";
            throw new ParserException(errorPosition, errorMessage);
          }
      }
    ...
    /**
     * Returns an array of symbols that can follow an instuction.
     */
