...
public class Parser
  {
     ...
     *
     * @return the parsed DeclarativePart.  Returns an empty
     *         DeclarativePart if parsing fails.
     */
    ...
    public List<InitialDecl> parseInitialDecls() throws IOException
      {
        List<InitialDecl> initialDecls = new LinkedList<>();
        ...
        try
          {
            ...
            typeDecl = new ArrayTypeDecl(typeId, elemType, numElements);
     ...
     *
     * @return a list of FormalParameters.  Returns an empty list if parsing fails.
     */
     ...
     *
     * @return the parsed StatementPart.  Returns a StatementPart
     *         with a null list of statements if parsing fails.
     */
        ...
        try
          {
            ...
            stmt = new ReturnStmt(subprogDecl, returnExpr, returnPosition);
            match(Symbol.semicolon);
          }
        catch (ParserException e)
          {
        ...
        try
          {
            ...
            var = new Variable(namedDecl, idToken.getPosition());
    ...
    public ConstValue parseConstValue() throws IOException
      {
        ...
        if (SymbolUtil.isLiteral(scanner.getSymbol()))
        ...
        else if (scanner.getSymbol() == Symbol.identifier)
          {
            ...
            InternalAssertion.check(decl != null && decl instanceof ConstDecl,
                "invalid declaration for constant " + constId.getText());

            constValue = new ConstValue(constId, (ConstDecl) decl);
          }
        else
    ...
    public NamedValue parseNamedValue() throws IOException
      {
        // Rather than call parseVariable() here, the code for parseVariable() is
        // essentially repeated.  The way that variable and named value are used in
        // CPRL, error recovery is different since they have different follow sets.
        ...
        try
          {
            ...
            if (decl == null)
                ...
                throw error("Identifier \"" + idToken + "\" is not a named value.");
    ...
    /**
     * Create an InternalCompilerException with the specified message and
